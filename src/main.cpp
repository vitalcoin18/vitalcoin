...
unsigned int nStakeMinAge = 60 * 60 * 24 * 1;	// minimum age for coin age: 30d
unsigned int nStakeMaxAge = 60 * 60 * 24 * 3;	// stake age of full weight: 90d
unsigned int nStakeTargetSpacing = 300;			// 120 sec block spacing
...
const string strMessageMagic = "VitalCoin Signed Message:\n";
...
void SyncWithWallets(const CTransaction& tx, const CBlock* pblock, bool fUpdate, bool fConnect)
{
    if (!fConnect)
    {
        // vitalcoin: wallets need to refund inputs when disconnecting coinstake
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
                if (pwallet->IsFromMe(tx))
                        ...
                        bool* pfMissingInputs)
{
    ...
    // vitalcoin: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
// vitalcoin: find block wanted by given orphan block
...
int64 GetProofOfWorkReward(int nHeight, int64 nFees, uint256 prevHash)
{
	...
	if(nHeight <= 10)
	{
		nSubsidy = 1000000 * COIN;	// Total coins, that all pow coins
...
int64 GetProofOfStakeReward(int64 nCoinAge, unsigned int nBits, unsigned int nTime, int nHeight)
{
	...
	if(nHeight < YEARLY_BLOCKCOUNT/2)
		nRewardCoinYear = 60 * MAX_MINT_PROOF_OF_STAKE;
	else if(nHeight < (YEARLY_BLOCKCOUNT))
		nRewardCoinYear = 40 * MAX_MINT_PROOF_OF_STAKE;
	else if(nHeight < (3 * YEARLY_BLOCKCOUNT)/2)
...
// vitalcoin: find last block index up to pindex
...
unsigned int GetNextTargetRequired(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    // vitalcoin: target change every block
    // vitalcoin: retarget with exponential moving toward target spacing
    CBigNum bnNew;
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // vitalcoin: check transaction timestamp
        ...
        if (IsCoinStake())
        {
            // vitalcoin: coin stake tx earns reward instead of paying fee
            uint64 nCoinAge;
        ...
        else
        {
            ...
            // vitalcoin: enforce transaction fees for every block
            if (nTxFee < GetMinFee())
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // vitalcoin: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in VitalCoin
    bool fStrictPayToScriptHash = true; // Always active in VitalCoin
    ...
    // vitalcoin: track money supply and mint amount info
    pindex->nMint = nValueOut - nValueIn + nFees;
    pindex->nMoneySupply = (pindex->pprev? pindex->pprev->nMoneySupply : 0) + nValueOut - nValueIn;
    if (!txdb.WriteBlockIndex(CDiskBlockIndex(pindex)))
        return error("Connect() : WriteBlockIndex for pindex failed");

    // vitalcoin: fees are not collected by miners as in bitcoin
    // vitalcoin: fees are destroyed to compensate the entire network
    if (fDebug && GetBoolArg("-printcreation"))
...
// vitalcoin: total coin age spent in transaction, in the unit of coin-days.
...
// vitalcoin: total coin age spent in block, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos)
{
    ...
    // vitalcoin: compute chain trust score
    pindexNew->bnChainTrust = (pindexNew->pprev ? pindexNew->pprev->bnChainTrust : 0) + pindexNew->GetBlockTrust();

    // vitalcoin: compute stake entropy bit for stake modifier
    if (!pindexNew->SetStakeEntropyBit(GetStakeEntropyBit(pindexNew->nHeight)))
        return error("AddToBlockIndex() : SetStakeEntropyBit() failed");

    // vitalcoin: record proof-of-stake hash value
    ...
    // vitalcoin: compute stake modifier
...
bool CBlock::CheckBlock(bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // vitalcoin: only the second transaction can be the optional coinstake
    for (unsigned int i = 2; i < vtx.size(); i++)
        if (vtx[i].IsCoinStake())
            return DoS(100, error("CheckBlock() : coinstake in wrong position"));

    // vitalcoin: coinbase output should be empty if proof-of-stake block
    ...
    // vitalcoin: check block signature
...
bool CBlock::AcceptBlock()
{
    ...
    // vitalcoin: check that the block satisfies synchronized checkpoint
    ...
    // vitalcoin: check pending sync-checkpoint
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // vitalcoin: check proof-of-stake
    ...
    // vitalcoin: verify hash target and signature of coinstake tx
    ...
    // vitalcoin: ask for pending sync-checkpoint if any
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom)
        {
            ...
            // vitalcoin: getblocks may not obtain the ancestor block rejected
    ...
    // vitalcoin: if responsible for sync-checkpoint send it
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "VitalCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "8th Jan PennyBase builds VitalCoin.";
        CTransaction txNew;
        ...
        block.nVersion = 1;
        block.nTime    = 1504875736;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 13291752;
        ...
        assert(block.hashMerkleRoot == uint256("0x91c8b66ff1add98422f934aab08d20d4681e08db6159905882b9dec747f17573"));
        ...
        // vitalcoin: initialize synchronized checkpoint
        if (!Checkpoints::WriteSyncCheckpoint((!fTestNet ? hashGenesisBlock : hashGenesisBlockTestNet)))
            return error("LoadBlockIndex() : failed to init sync checkpoint");
    }

    // vitalcoin: if checkpoint master key changed must reset sync-checkpoint
    {
        CTxDB txdb;
        string strPubKey = "";
...
string GetWarnings(string strFor)
{
    ...
    // vitalcoin: wallet lock warning for minting
    if (strMintWarning != "")
    {
        nPriority = 0;
    ...
    // vitalcoin: should not enter safe mode for longer invalid chain
    // vitalcoin: if sync-checkpoint is too old do not enter safe mode
    ...
    // vitalcoin: if detected invalid checkpoint enter safe mode
    ...
    // Alerts
    {
        ...
        BOOST_FOREACH(PAIRTYPE(const uint256, CAlert)& item, mapAlerts)
        {
            ...
            if (alert.AppliesToMe() && alert.nPriority > nPriority)
            {
                ...
                if (nPriority > 1000)
                    strRPC = strStatusBar;  // vitalcoin: safe mode for high alert
            }
        }
    }
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "version")
    {
        ...
        // vitalcoin: record my external IP reported by peer
        ...
        // vitalcoin: relay sync-checkpoint
        {
        ...
        BOOST_FOREACH(const CInv& inv, vInv)
        {
            ...
            if (inv.type == MSG_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    {
                        // vitalcoin: send latest proof-of-work block to allow the
    ...
    else if (strCommand == "getblocks")
    {
        ...
        for (; pindex; pindex = pindex->pnext)
        {
            if (pindex->GetBlockHash() == hashStop)
            {
                ...
                // vitalcoin: tell downloading node about the latest block if it's
...
CBlock* CreateNewBlock(CWallet* pwallet, bool fProofOfStake)
{
    ...
    // vitalcoin: if coinstake available add coinstake tx
    ...
    int64 nFees = 0;
    {
        ...
        while (!vecPriority.empty())
        {
            ...
            // vitalcoin: simplify transaction fee - allow free = false
...
void BitcoinMiner(CWallet *pwallet, bool fProofOfStake)
{
    ...
    while (fGenerateBitcoins || fProofOfStake)
    {
        ...
        if (fProofOfStake)
        {
            // vitalcoin: if proof-of-stake block found then process block
            if (pblock->IsProofOfStake())
            {
