...
// vitalcoin: optional setting to unlock wallet for block minting only;
...
// vitalcoin: total coins staked (non-spendable until maturity)
...
bool CWallet::SelectCoinsMinConf(int64 nTargetValue, unsigned int nSpendTime, int nConfMine, int nConfTheirs, vector<COutput> vCoins, set<pair<const CWalletTx*,unsigned int> >& setCoinsRet, int64& nValueRet) const
{
    ...
    BOOST_FOREACH(COutput output, vCoins)
    {
        ...
        if (pcoin->nTime > nSpendTime)
            continue;  // vitalcoin: timestamp must not exceed spend time
...

    {
        ...
        CTxDB txdb("r");
        {
            ...
            while(true)
            {
                ...
                // vitalcoin: sub-cent change is moved to fee
...
// vitalcoin: create coin stake transaction
...
// vitalcoin: check 'spent' consistency between wallet and txindex
// vitalcoin: fix wallet spent state according to txindex
...
// vitalcoin: disable transaction (only for coinstake)
